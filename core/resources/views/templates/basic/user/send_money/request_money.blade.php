...
@section('content')
    <div>
        <h1>{{ $pageTitle }}</h1>
        <form class="requestMoneyForm" method="post" action="{{ route('user.store.money.request') }}">
            @csrf
            <div class="form-group">
                <label class="form-label"> @lang('Username')</label>
                <input type="text" name="username" value="{{ old('username') }}" placeholder="@lang('Enter username')"
                    class="form-control  form--control usernameInput" required>
                <p id="userDetails" class="text-success mt-2"></p>
            </div>
            <div class="form-group">
                <label class="form-label"> @lang('Amount')</label>
                <input type="number" name="amount" value="{{ old('amount') }}" placeholder="@lang('Enter amount')"
            ...
            <div class="form-group">
                <button class="btn btn-sm btn--base requestMoneyBtn">@lang('Request Money')</button>
            </div>
        </form>
    </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>@lang('Are you sure you want to request money')?</p>
                </div>
...
@push('script-lib')
    <script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $('#userDetails').removeClass('text-danger mt-2').addClass(
                                    'text-success mt-2');
                            } else {
                                $('#userDetails').text(response.message);
                                $('#userDetails').removeClass('text-success mt-2').addClass(
                                    'text-danger mt-2');
                            }
                        },
                        error: function() {
        ...
        $('[name="amount"]').on('input', function() {
            ...
            let fixedCharge = parseFloat('{{ gs('request_money_fixed_charge') }}');
            let percentCharge = parseFloat('{{ gs('request_money_percent_charge') }}');
            ...
            if (amount > 0) {
                ...
                $('li:has(span:contains("Charge")) span:last-child').text(charge.toFixed(2) +
                    ' {{ __(gs('cur_text')) }}');
                $('li:has(span:contains("Amount")) span:last-child').text(amount.toFixed(2) +
                    ' {{ __(gs('cur_text')) }}');
                $('li:has(span:contains("Final Amount")) span:last-child').text(finalAmount.toFixed(2) +
                    ' {{ __(gs('cur_text')) }}');
            } else {
